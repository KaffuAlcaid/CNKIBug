...
勾选论文后，在窗口上方的“导出类型”中选择 Excel (.xlsx)、CSV (.csv) 或 Zotero (.ris)，再点击“导出所选”。程序自动命名文件并保存到主窗口设置的目录，窗口底部显示保存位置和导出结果。

Zotero 文件可通过 Zotero 的“文件 → 导入”读取，命中检索项保存在文献备注中；勾选“包含已下载附件”可关联已经下载的 PDF。导入时应保持 PDF 文件路径有效。
