...
def _scrape_keyword(page, keyword: str, max_pages: int) -> list:
    ...
    try:
        ...
        _console.print("[yellow][!] 用户中断，跳过该关键词。[/yellow]")
        return results
...
def scrape_cnki(keywords: list[str], max_pages: int, save_mode: str):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # v0.1.4: 预热搜索——用固定关键词触发一次完整搜索流程，消耗首次 302 重定向
            dummy_keyword = "焊接"
            try:
                # 使用 rich 的 status 生成“少女祈祷中”动画，spinner 可选 "dots", "bouncingBar", "point" 等
                with _console.status("[bold magenta]少女祈祷中... [/bold magenta]",
                                     spinner="bouncingBar"):
                    page.goto("https://www.cnki.net/", timeout=30000)
                    page.wait_for_load_state("domcontentloaded", timeout=20000)
                    _check_captcha(page)
